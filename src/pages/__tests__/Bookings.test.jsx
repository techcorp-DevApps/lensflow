...
describe('Bookings page', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.spyOn(apiClient.auth, 'me').mockResolvedValue({ email: 'photog@example.com' });
  });

  test('shows skeletons while loading', () => {
    vi.spyOn(apiClient.entities.Booking, 'list').mockImplementation(() => new Promise(() => {}));
  ...
  test('renders populated booking list', async () => {
    vi.spyOn(apiClient.entities.Booking, 'list').mockResolvedValue([
  ...
  test('shows error state when list fails', async () => {
    vi.spyOn(apiClient.entities.Booking, 'list').mockRejectedValue(
